...
<x-app-layout>
    <x-slot name="header">
        ...
        <div class=" inline-block absolute right-60 text-white text-lg rounded px-4 py-1 bg-blue-400" x-data="{create:false}">
            <button class="block" @click="create=!create">Add a Category</button>

            <form action="{{route('admin.storePortfolioCategory')}}"
                  x-show="create"
                  method="POST"
                ...
                <div class="p-6 text-gray-900">
                    ...
                    @foreach($categories as $category)
                        <div x-data="{show:false}" class=" inline-block m-2 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                            <h5 class="mb-2 text-lg font-bold tracking-tight text-white ">{{ ucwords($category->name)}}</h5>
                            <div class="flex sm:justify-start  space-x-4 mt-3">
                                ...
                                <form action="{{route('admin.deletePortfolioCategory' , $category->name)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <form x-show="show" action="{{route('admin.updatePortfolioCategory' , $category->name)}}" method="POST" class="mt-6 flex space-x-10">
                                @csrf
                                @method('PUT')
